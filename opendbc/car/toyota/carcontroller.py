...
from opendbc.car.toyota.values import CAR, STATIC_DSU_MSGS, NO_STOP_TIMER_CAR, TSS2_CAR, \
                                        CarControllerParams, ToyotaFlags, ToyotaFlagsSP, \
                                        UNSUPPORTED_DSU_CAR
from opendbc.can.packer import CANPacker

from opendbc.sunnypilot.car.toyota.carcontroller import SPController
...
class CarController(CarControllerBase):
  def __init__(self, dbc_names, CP):
    ...
    self.sp_controller = SPController(CP)
  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.CP.spFlags & ToyotaFlagsSP.SP_AUTO_BRAKE_HOLD:
      can_sends.extend(self.sp_controller.create_auto_brake_hold_messages(CS))
    ...
    if self.CP.spFlags & ToyotaFlagsSP.SP_ENHANCED_BSM and self.frame > 200:
      can_sends.extend(self.sp_controller.create_enhanced_bsm_messages(CS, 20, True))
